...
DECLARE
    ...
    --  GET THE METADATA_ID FROM THE _METADATA.MSSQL_SOURCES TABLE
    --
    --  SELECT * FROM MIGRATESQLAPP._METADATA.MSSQL_SOURCES ORDER BY UPDATED DESC LIMIT 20;
    
    METADATA_ID INT DEFAULT 1101;   -- SET THIS VALUE SO YOU KNOW WHAT SCHEMA TO CREATE

    MSSQL_SOURCE_ID VARCHAR(36);

    SCHEMA_CURSOR CURSOR FOR SELECT X.SCHEMA_DDL, 
          FROM MIGRATESQLAPP._METADATA.MSSQL_SCHEMAS X
          WHERE X.MSSQL_SOURCE_ID = ?;
...
BEGIN
    ...
    SELECT MSSQL_SOURCE_ID INTO :MSSQL_SOURCE_ID
        FROM MIGRATESQLAPP._METADATA.MSSQL_SOURCES
        WHERE METADATA_ID = :METADATA_ID;


    OPEN SCHEMA_CURSOR USING (:MSSQL_SOURCE_ID);
    ...
    OPEN TABLE_CURSOR USING (:MSSQL_SOURCE_ID);
    ...
    OPEN VIEW_CURSOR USING (:MSSQL_SOURCE_ID);
    ...
    OPEN FOREIGN_KEY_CURSOR USING (:MSSQL_SOURCE_ID);

    FOR _FOREIGN_KEY_RECORD IN FOREIGN_KEY_CURSOR DO

        DDL := _FOREIGN_KEY_RECORD.FOREIGN_KEY_DDL;
